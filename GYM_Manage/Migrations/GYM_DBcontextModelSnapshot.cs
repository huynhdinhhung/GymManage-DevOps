// <auto-generated />
using System;
using GYM_Manage.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace GYM_Manage.Migrations
{
    [DbContext(typeof(GYM_DBcontext))]
    partial class GYM_DBcontextModelSnapshot : ModelSnapshot
    {
        protected override void BuildModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("GYM_Manage.Models.BaiViet", b =>
                {
                    b.Property<int>("MaBaiViet")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaBaiViet"));

                    b.Property<string>("HinhAnh")
                        .IsRequired()
                        .HasMaxLength(555)
                        .HasColumnType("nvarchar(555)");

                    b.Property<int>("IDNguoiTao")
                        .HasColumnType("int");

                    b.Property<string>("MoTaNgan")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTime?>("NgayCapNhat")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("NgayDang")
                        .HasColumnType("datetime2");

                    b.Property<string>("NoiDung")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TieuDe")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaBaiViet");

                    b.HasIndex("IDNguoiTao");

                    b.ToTable("BaiViets");
                });

            modelBuilder.Entity("GYM_Manage.Models.CaLamViec", b =>
                {
                    b.Property<int>("MaCa")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCa"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<TimeSpan>("GioBatDau")
                        .HasColumnType("time");

                    b.Property<TimeSpan>("GioKetThuc")
                        .HasColumnType("time");

                    b.Property<string>("TenCa")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("MaCa");

                    b.ToTable("CaLamViec");
                });

            modelBuilder.Entity("GYM_Manage.Models.DangKyGoiTap", b =>
                {
                    b.Property<int>("MaDangKy")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDangKy"));

                    b.Property<int?>("MaCa")
                        .HasColumnType("int");

                    b.Property<int>("MaGoiTap")
                        .HasColumnType("int");

                    b.Property<int?>("MaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<int>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayBatDau")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("NgayKetThuc")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("NgayTap")
                        .HasColumnType("datetime2");

                    b.Property<int?>("ThuTrongTuan")
                        .HasColumnType("int");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaDangKy");

                    b.HasIndex("MaCa");

                    b.HasIndex("MaGoiTap");

                    b.HasIndex("MaHuanLuyenVien");

                    b.HasIndex("MaThanhVien");

                    b.ToTable("DangKyGoiTaps");
                });

            modelBuilder.Entity("GYM_Manage.Models.GoiTap", b =>
                {
                    b.Property<int>("MaGoiTap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaGoiTap"));

                    b.Property<string>("AnhDemo")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<decimal>("GiaTien")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("LoaiGoi")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("MoTa")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("TenGoiTap")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<int>("ThoiHan")
                        .HasColumnType("int");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("MaGoiTap");

                    b.ToTable("GoiTaps");
                });

            modelBuilder.Entity("GYM_Manage.Models.HoaDon", b =>
                {
                    b.Property<int>("MaHoaDon")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHoaDon"));

                    b.Property<int>("MaThanhToan")
                        .HasColumnType("int");

                    b.Property<int>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayHoaDon")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("TongSoTien")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaHoaDon");

                    b.HasIndex("MaThanhToan");

                    b.HasIndex("MaThanhVien");

                    b.ToTable("HoaDons");
                });

            modelBuilder.Entity("GYM_Manage.Models.HuanLuyenVien", b =>
                {
                    b.Property<int>("MaHuanLuyenVien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHuanLuyenVien"));

                    b.Property<string>("AnhDaiDien")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ChungChi")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("ChuyenMon")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("HoTen")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayTuyenDung")
                        .HasColumnType("datetime2");

                    b.Property<int?>("NguoiDungMaNguoiDung")
                        .HasColumnType("int");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("MaHuanLuyenVien");

                    b.HasIndex("NguoiDungMaNguoiDung");

                    b.ToTable("HuanLuyenViens");
                });

            modelBuilder.Entity("GYM_Manage.Models.LichLamViecHLV", b =>
                {
                    b.Property<int>("MaLich")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLich"));

                    b.Property<int?>("HuanLuyenVienThayTheMaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<bool>("IsThayThe")
                        .HasColumnType("bit");

                    b.Property<string>("LyDoThayDoi")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int>("MaCa")
                        .HasColumnType("int");

                    b.Property<int>("MaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<int?>("MaHuanLuyenVienThayThe")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayLamViec")
                        .HasColumnType("datetime2");

                    b.Property<int>("ThuTrongTuan")
                        .HasColumnType("int");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("MaLich");

                    b.HasIndex("HuanLuyenVienThayTheMaHuanLuyenVien");

                    b.HasIndex("MaCa");

                    b.HasIndex("MaHuanLuyenVien");

                    b.ToTable("LichLamViecHLVs");
                });

            modelBuilder.Entity("GYM_Manage.Models.LichTapThanhVien", b =>
                {
                    b.Property<int>("MaLichTap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichTap"));

                    b.Property<int?>("HuanLuyenVienMaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<bool>("IsHLVThayThe")
                        .HasColumnType("bit");

                    b.Property<int>("MaCa")
                        .HasColumnType("int");

                    b.Property<int>("MaDangKy")
                        .HasColumnType("int");

                    b.Property<int>("MaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<int?>("MaHuanLuyenVienThayThe")
                        .HasColumnType("int");

                    b.Property<int>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayTap")
                        .HasColumnType("datetime2");

                    b.HasKey("MaLichTap");

                    b.HasIndex("HuanLuyenVienMaHuanLuyenVien");

                    b.HasIndex("MaCa");

                    b.HasIndex("MaDangKy");

                    b.HasIndex("MaHuanLuyenVien");

                    b.HasIndex("MaHuanLuyenVienThayThe");

                    b.HasIndex("MaThanhVien");

                    b.ToTable("LichTapThanhViens");
                });

            modelBuilder.Entity("GYM_Manage.Models.NguoiDung", b =>
                {
                    b.Property<int>("MaNguoiDung")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNguoiDung"));

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("HoTen")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("IsVip")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("LanDangNhapCuoi")
                        .HasColumnType("datetime2");

                    b.Property<string>("MatKhau")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2");

                    b.Property<string>("TenDangNhap")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("TongChiTieu")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("VaiTro")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("VipSinceUtc")
                        .HasColumnType("datetime2");

                    b.HasKey("MaNguoiDung");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("TenDangNhap")
                        .IsUnique();

                    b.ToTable("NguoiDungs");
                });

            modelBuilder.Entity("GYM_Manage.Models.OtpCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("ExpiredAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Used")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("OtpCodes");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThanhToan", b =>
                {
                    b.Property<int>("MaThanhToan")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaThanhToan"));

                    b.Property<int>("MaDangKyGoiTap")
                        .HasColumnType("int");

                    b.Property<int>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayThanhToan")
                        .HasColumnType("datetime2");

                    b.Property<string>("PhuongThucThanhToan")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<decimal>("SoTien")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("MaThanhToan");

                    b.HasIndex("MaDangKyGoiTap");

                    b.HasIndex("MaThanhVien");

                    b.ToTable("ThanhToans");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThanhVien", b =>
                {
                    b.Property<int>("MaThanhVien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaThanhVien"));

                    b.Property<string>("DiaChi")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("GioiTinh")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int?>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgaySinh")
                        .HasColumnType("datetime2");

                    b.Property<string>("SoDienThoai")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("MaThanhVien");

                    b.HasIndex("MaNguoiDung");

                    b.HasIndex("SoDienThoai")
                        .IsUnique()
                        .HasFilter("[SoDienThoai] IS NOT NULL");

                    b.ToTable("ThanhViens");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThietBi", b =>
                {
                    b.Property<int>("MaThietBi")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaThietBi"));

                    b.Property<string>("DanhMuc")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTime?>("NgayBaoTriCuoi")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("NgayMua")
                        .HasColumnType("datetime2");

                    b.Property<string>("TenThietBi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaThietBi");

                    b.ToTable("ThietBis");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThongBao", b =>
                {
                    b.Property<int>("MaThongBao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaThongBao"));

                    b.Property<bool>("DaDoc")
                        .HasColumnType("bit");

                    b.Property<bool>("DaXem")
                        .HasColumnType("bit");

                    b.Property<bool>("IsImportant")
                        .HasColumnType("bit");

                    b.Property<string>("LoaiThongBao")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("MaHuanLuyenVien")
                        .HasColumnType("int");

                    b.Property<int?>("MaHuanLuyenVienThayThe")
                        .HasColumnType("int");

                    b.Property<int?>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<int?>("MaYeuCauDoiLich")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayGui")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2");

                    b.Property<string>("NoiDung")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("TieuDe")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("MaThongBao");

                    b.HasIndex("MaHuanLuyenVien");

                    b.HasIndex("MaHuanLuyenVienThayThe");

                    b.HasIndex("MaThanhVien");

                    b.HasIndex("MaYeuCauDoiLich");

                    b.ToTable("ThongBaos");
                });

            modelBuilder.Entity("GYM_Manage.Models.YeuCauDoiLich", b =>
                {
                    b.Property<int>("MaYeuCau")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaYeuCau"));

                    b.Property<string>("LoaiDoi")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("LyDo")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("MaHlvGui")
                        .HasColumnType("int");

                    b.Property<int>("MaHlvNhan")
                        .HasColumnType("int");

                    b.Property<int>("MaLichGui")
                        .HasColumnType("int");

                    b.Property<int?>("MaLichNhan")
                        .HasColumnType("int");

                    b.Property<int?>("MaThanhVien")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayGui")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("NgayTao")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("NgayXuLy")
                        .HasColumnType("datetime2");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("MaYeuCau");

                    b.HasIndex("MaHlvGui");

                    b.HasIndex("MaHlvNhan");

                    b.HasIndex("MaLichGui");

                    b.HasIndex("MaLichNhan");

                    b.HasIndex("MaThanhVien");

                    b.ToTable("YeuCauDoiLichs");
                });

            modelBuilder.Entity("GYM_Manage.Models.BaiViet", b =>
                {
                    b.HasOne("GYM_Manage.Models.NguoiDung", "NguoiTao")
                        .WithMany()
                        .HasForeignKey("IDNguoiTao")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NguoiTao");
                });

            modelBuilder.Entity("GYM_Manage.Models.DangKyGoiTap", b =>
                {
                    b.HasOne("GYM_Manage.Models.CaLamViec", "CaLamViec")
                        .WithMany()
                        .HasForeignKey("MaCa")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("GYM_Manage.Models.GoiTap", "GoiTap")
                        .WithMany("DangKyGoiTaps")
                        .HasForeignKey("MaGoiTap")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVien")
                        .WithMany("DangKyGoiTaps")
                        .HasForeignKey("MaHuanLuyenVien")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany("DangKyGoiTaps")
                        .HasForeignKey("MaThanhVien")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("CaLamViec");

                    b.Navigation("GoiTap");

                    b.Navigation("HuanLuyenVien");

                    b.Navigation("ThanhVien");
                });

            modelBuilder.Entity("GYM_Manage.Models.HoaDon", b =>
                {
                    b.HasOne("GYM_Manage.Models.ThanhToan", "ThanhToan")
                        .WithMany()
                        .HasForeignKey("MaThanhToan")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany()
                        .HasForeignKey("MaThanhVien")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ThanhToan");

                    b.Navigation("ThanhVien");
                });

            modelBuilder.Entity("GYM_Manage.Models.HuanLuyenVien", b =>
                {
                    b.HasOne("GYM_Manage.Models.NguoiDung", "NguoiDung")
                        .WithMany()
                        .HasForeignKey("NguoiDungMaNguoiDung");

                    b.Navigation("NguoiDung");
                });

            modelBuilder.Entity("GYM_Manage.Models.LichLamViecHLV", b =>
                {
                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVienThayThe")
                        .WithMany()
                        .HasForeignKey("HuanLuyenVienThayTheMaHuanLuyenVien");

                    b.HasOne("GYM_Manage.Models.CaLamViec", "CaLamViec")
                        .WithMany("LichLamViecs")
                        .HasForeignKey("MaCa")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVien")
                        .WithMany("LichLamViecs")
                        .HasForeignKey("MaHuanLuyenVien")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CaLamViec");

                    b.Navigation("HuanLuyenVien");

                    b.Navigation("HuanLuyenVienThayThe");
                });

            modelBuilder.Entity("GYM_Manage.Models.LichTapThanhVien", b =>
                {
                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", null)
                        .WithMany("LichTapThanhViens")
                        .HasForeignKey("HuanLuyenVienMaHuanLuyenVien");

                    b.HasOne("GYM_Manage.Models.CaLamViec", "CaLamViec")
                        .WithMany()
                        .HasForeignKey("MaCa")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.DangKyGoiTap", "DangKyGoiTap")
                        .WithMany("LichTapThanhViens")
                        .HasForeignKey("MaDangKy")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVien")
                        .WithMany()
                        .HasForeignKey("MaHuanLuyenVien")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVienThayThe")
                        .WithMany()
                        .HasForeignKey("MaHuanLuyenVienThayThe")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany("LichTapThanhViens")
                        .HasForeignKey("MaThanhVien")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("CaLamViec");

                    b.Navigation("DangKyGoiTap");

                    b.Navigation("HuanLuyenVien");

                    b.Navigation("HuanLuyenVienThayThe");

                    b.Navigation("ThanhVien");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThanhToan", b =>
                {
                    b.HasOne("GYM_Manage.Models.DangKyGoiTap", "DangKyGoiTap")
                        .WithMany("ThanhToans")
                        .HasForeignKey("MaDangKyGoiTap")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany("ThanhToans")
                        .HasForeignKey("MaThanhVien")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("DangKyGoiTap");

                    b.Navigation("ThanhVien");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThanhVien", b =>
                {
                    b.HasOne("GYM_Manage.Models.NguoiDung", "NguoiDung")
                        .WithMany("ThanhViens")
                        .HasForeignKey("MaNguoiDung");

                    b.Navigation("NguoiDung");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThongBao", b =>
                {
                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVien")
                        .WithMany("ThongBaos")
                        .HasForeignKey("MaHuanLuyenVien")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HuanLuyenVienThayThe")
                        .WithMany()
                        .HasForeignKey("MaHuanLuyenVienThayThe");

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany("ThongBaos")
                        .HasForeignKey("MaThanhVien")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("GYM_Manage.Models.YeuCauDoiLich", "YeuCauDoiLich")
                        .WithMany()
                        .HasForeignKey("MaYeuCauDoiLich")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("HuanLuyenVien");

                    b.Navigation("HuanLuyenVienThayThe");

                    b.Navigation("ThanhVien");

                    b.Navigation("YeuCauDoiLich");
                });

            modelBuilder.Entity("GYM_Manage.Models.YeuCauDoiLich", b =>
                {
                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HlvGui")
                        .WithMany("YeuCauDoiLichGui")
                        .HasForeignKey("MaHlvGui")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.HuanLuyenVien", "HlvNhan")
                        .WithMany("YeuCauDoiLichNhan")
                        .HasForeignKey("MaHlvNhan")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.LichLamViecHLV", "LichGui")
                        .WithMany()
                        .HasForeignKey("MaLichGui")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GYM_Manage.Models.LichLamViecHLV", "LichNhan")
                        .WithMany()
                        .HasForeignKey("MaLichNhan")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("GYM_Manage.Models.ThanhVien", "ThanhVien")
                        .WithMany()
                        .HasForeignKey("MaThanhVien");

                    b.Navigation("HlvGui");

                    b.Navigation("HlvNhan");

                    b.Navigation("LichGui");

                    b.Navigation("LichNhan");

                    b.Navigation("ThanhVien");
                });

            modelBuilder.Entity("GYM_Manage.Models.CaLamViec", b =>
                {
                    b.Navigation("LichLamViecs");
                });

            modelBuilder.Entity("GYM_Manage.Models.DangKyGoiTap", b =>
                {
                    b.Navigation("LichTapThanhViens");

                    b.Navigation("ThanhToans");
                });

            modelBuilder.Entity("GYM_Manage.Models.GoiTap", b =>
                {
                    b.Navigation("DangKyGoiTaps");
                });

            modelBuilder.Entity("GYM_Manage.Models.HuanLuyenVien", b =>
                {
                    b.Navigation("DangKyGoiTaps");

                    b.Navigation("LichLamViecs");

                    b.Navigation("LichTapThanhViens");

                    b.Navigation("ThongBaos");

                    b.Navigation("YeuCauDoiLichGui");

                    b.Navigation("YeuCauDoiLichNhan");
                });

            modelBuilder.Entity("GYM_Manage.Models.NguoiDung", b =>
                {
                    b.Navigation("ThanhViens");
                });

            modelBuilder.Entity("GYM_Manage.Models.ThanhVien", b =>
                {
                    b.Navigation("DangKyGoiTaps");

                    b.Navigation("LichTapThanhViens");

                    b.Navigation("ThanhToans");

                    b.Navigation("ThongBaos");
                });
#pragma warning restore 612, 618
        }
    }
}
